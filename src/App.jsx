    ...
    return (
        <div className="relative w-full" ref={containerRef}>
            <button 
                ...
                className={`w-full p-2.5 rounded-xl border text-[10px] font-black transition-all flex items-center justify-between gap-2 text-center
...
export default function App() {
    ...
    const calculateProgress = (ship) => {
        ...
        processes.forEach(proc => {
            ...
            const selected = normalizeStatus(ship.status?.[proc.id]);
                                                            ...
                                                            return (
                                                                <tr key={ship.id} className={`hover:bg-slate-50/50 transition-colors ${isSelected ? 'bg-blue-50/30' : ''}`}>
                                                                    ...
                                                                    {processes.map(p => {
                                                                        ...
                                                                        const statusArray = normalizeStatus(rawStatus);
                                                                        return (
